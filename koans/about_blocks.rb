...
class AboutBlocks < Neo::Koan
  ...
  def test_methods_can_take_blocks
    yielded_result = method_with_block { return 1 + 2 }
    assert_equal 3, yielded_result
  end

  def test_blocks_can_be_defined_with_do_end_too
    yielded_result = method_with_block do return 1 + 2 end
    assert_equal 3, yielded_result
  end
  ...
  def test_block_can_affect_variables_in_the_code_where_they_are_created
    ...
    #дело в том, что блоки не создают свой контекст.
    # поэтому соответственные переменные подхватываются извне
  end
