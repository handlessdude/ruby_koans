...
EOS
  ...
  def test_the_shovel_operator_modifies_the_original_string
    ...
    #
    # So << alters the original string rather than creating a new one. The reason for this is that in ruby a += b is syntactic shorthand for a = a + b (the same goes for the other <op>= operators) which is an assignment.
  ...
  def test_single_quotes_sometimes_interpret_escape_characters
    string = '\\\''
    # one slash shields single quote, 
    # another slash shields one more slash
    puts string
    assert_equal 2, string.size
    assert_equal "\\'", string
  end
